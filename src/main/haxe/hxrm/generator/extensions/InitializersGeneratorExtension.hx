...
class InitializersGeneratorExtension extends GeneratorExtensionBase {
	...
	function parseBindingInitializator(context:HxmrContext, scope:GeneratorScope, fieldName:String, initializator : BindingInitializator) : Expr {
		var value = null;
		try {
		...
		var field = scope.context.node.getFieldByName(fieldName);
