...
class InitializersGeneratorExtension extends GeneratorExtensionBase {
	...
	function processScope(context : HxmrContext, scope : GeneratorScope, nodeScope : NodeScope, exprs : Array<Expr>, forField : String) : Void {
		...
		for (fieldName in initializers.keys()) {
			switch(initializers.get(fieldName)) {
				case InitBinding(initializator):
					processBindingInitializator(context, scope, nodeScope, fieldName, initializator, exprs, macro $i{forField}.$fieldName);
				case InitNodeScope(initializator):
					processBindingInitializator(context, scope, nodeScope, fieldName, initializator, exprs, macro $i{forField}.$fieldName);
			}
			//trace("\n" + (new Printer("   ")).printTypeDefinition(scope.typeDefinition, true));
	...
	function processBindingInitializator(context:HxmrContext, scope:GeneratorScope, nodeScope : NodeScope, fieldName : String, initializator : BindingInitializator, exprs : Array<Expr>, assignTarget : Expr) : Void {

		var res = parseBindingInitializator(context, scope, nodeScope, fieldName, initializator, exprs);
	...
	function parseBindingInitializator(context:HxmrContext, scope:GeneratorScope, nodeScope : NodeScope, fieldName : String, initializator : BindingInitializator, exprs : Array<Expr>) : Expr {
		...
		return switch([fieldClassType.module, fieldClassType.name]) {
			...
			case ["Array", "Array"]:
				...
				for(childInitializer in initScope.children) {
					values.push(macro $i {generateInitializerName(childInitializer.fieldName)}());
				}
				{
